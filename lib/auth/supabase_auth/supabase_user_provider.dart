...
class GabitechSupabaseUser extends BaseAuthUser {
  GabitechSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> gabitechSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = GabitechSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
